...
int Child1(char *my_name) 
{
   ...
   for (j = 0; j < 3; j++) {
      for (i = 0; i < 1000; i++)
         temp = 2 + temp;
      GetTimeofDay(&time);
      USLOSS_Console("%s(): current time of day = %3d   Should be close, but does not have to be an exact match\n", my_name, time);

      CPUTime(&time);
      USLOSS_Console("%s(): current CPU time = %3d     Should be close, but does not have to be an exact match\n", my_name, time);
   }
